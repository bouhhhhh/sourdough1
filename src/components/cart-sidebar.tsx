		...
		<>
			...
			<div className="fixed right-0 top-0 z-50 h-screen w-96 max-w-full bg-white shadow-xl transition-transform">
				<div className="flex h-full flex-col relative">
							...
							<div className="flex h-full flex-col items-center justify-center gap-4 p-6 text-center">
								...
								<div>
									...
									<Link
										...
										onClick={onClose}
										className="inline-flex items-center gap-1 mt-3 text-sm font-medium text-red-600 hover:text-red-700 underline"
									>
										Discover our sourdough starter
					...
					{/* Footer with Total and Buttons - When cart has items */}
					{cart && cart.items.length > 0 && (
						<div className="border-t p-4 space-y-4">
							...
							<div className="flex gap-3">
								...
								<button
									onClick={onClose}
									className="flex-1 text-center rounded-lg bg-white border-2 border-gray-300 px-6 py-3 text-gray-900 font-semibold hover:bg-gray-50 transition-colors"
								...
								<Link
									href="/checkout"
									className="flex-1 text-center rounded-lg bg-black px-6 py-3 text-white font-semibold hover:bg-gray-800 transition-colors"
									onClick={onClose}
								>
									Checkout
								</Link>
					...
					{/* Footer with only Continue Shopping - When cart is empty */}
					{(!cart || !cart.items || cart.items.length === 0) && (
						<div className="border-t p-4">
							<button
								onClick={onClose}
								className="w-full text-center rounded-lg bg-black px-6 py-3 text-white font-semibold hover:bg-gray-800 transition-colors"
								aria-label="Continue shopping"
							>
								Continue Shopping
							</button>
